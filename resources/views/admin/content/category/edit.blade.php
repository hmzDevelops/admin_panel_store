                    ...
                    <form id="form" action="{{ route('admin.content.category.update', $postCategory) }}" method="post"
                        ...
                        <section class="row">
                            ...
                            <section class="col-12 col-md-6 form-group">
                                ...
                                <input value="{{ old('name', $postCategory->name) }}" type="text" name="name"
                            ...
                            <section class="col-12 col-md-6 form-group">
                                ...
                                <input value="{{ old('tags', $postCategory->tags) }}" type="hidden" name="tags"
                            ...
                            <section class="col-12 col-md-6 form-group">
                                ...
                                <select name="status" id="status" class="form-control form-control-sm">
                                    <option value="0" @if (old('description', $postCategory->status) == 0) selected @endif>غیر فعال
                                    </option>
                                    <option value="1" @if (old('description', $postCategory->status) == 1) selected @endif>فعال</option>
                                </select>
                                ...
                                <section class="row">
                                    ...
                                    @foreach ($postCategory->image['indexArray'] as $key => $value)
                                        <section class="form-check col-md-4">
                                            ...
                                            <input type="radio" name="currentImage" id="{{ $number }}" value="{{ $key }}" @if ($postCategory->image['currentImage'] == $key)
                                            checked @endif>

                                            <label for="{{ $number }}">@if($key == "large") بزرگ @elseif ($key == "medium") متوسط @else کوچک @endif
                                                <img src="{{ asset($value) }}" alt="avatar" width="80" height="80">
                                            </label>
                                        </section>
