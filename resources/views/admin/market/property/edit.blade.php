...
@section('page-title')
    <title>{{ config('constants.page_title.edit_property') }}</title>
@endsection
                    ...
                    <form action="{{ route('admin.market.property.update', $category_attribute) }}" method="post">
                        ...
                        <section class="row">
                            ...
                            <section class="col-12 col-md-6 form-group">
                                ...
                                <input type="text" name="name" value="{{ old('name', $category_attribute->name) }}"
                            ...
                            <section class="col-12 col-md-6 form-group">
                                ...
                                <input type="text" name="unit" value="{{ old('unit', $category_attribute->unit) }}"
                                    class="form-control form-control-sm">
                                @error('unit')
                                    <span class="alert alert-danger invalid-feedback d-block" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                <select name="category_id" id="category_id" class="form-control form-control-sm">
                                    @foreach ($productCategorys as $productCategory)
                                        <option value="{{ $productCategory->id }}"
                                            @if (old('category_id', $category_attribute->category_id) == $productCategory->id) selected @endif>{{ $productCategory->name }}
                                        </option>
                                    @endforeach
                                </select>
