...
class PaymentController extends Controller
{
    ...
    public function canceled(Payment $payment)
    {
        $payment->status = 2;
        $payment->save();
        return redirect()->route('admin.market.payment.index')->with('swal-success','پرداخت فوق با موفقیت باطل شد');
    }
    ...
    public function returned(Payment $payment)
    {
        $payment->status = 3;
        $payment->save();
        return redirect()->route('admin.market.payment.index')->with('swal-success','پرداخت فوق با موفقیت برگشت داده شد');
    }
